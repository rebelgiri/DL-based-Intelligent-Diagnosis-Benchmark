...
def get_files(root, test=False):
    ...
    '''
    datasetname = os.listdir(root)  # 0:training, 2:test
    training_data = os.path.join(root, datasetname[0]) #Path of training
    testing_data = os.path.join(root, datasetname[1]) #Path of test

    classes_labels = os.listdir(training_data)

    
    data = []
    lab =[]
    for i in range(len(classes_labels)):
        logging.info("Class {} labeled as : {}".format(classes_labels[i], i))
        class_path = os.path.join(training_data , classes_labels[i])
...
def data_load(filename,dataname,label):
    ...
    for line in islice(f, 29, None):  #Skip the first 16 lines
        line = line.rstrip()
        word = line.split(";", 8)   #Separated by ;
        fl.append(eval(word[1]))   # Take a vibration signal in the x direction as input
